...
void drive_forward() { 
  ...
  analogWrite(left_pwm,255); //Can edit the encoder value if one car being faster/slower is preferrable
  analogWrite(right_pwm,255); //But in the most efficient smart traffic system, all cars are going the same maximum speed
...
void drive_slow_forward() {
  ...
  delay(200);
  analogWrite(left_pwm,123); //Realistic systems would have cars speed up at yellow lights but
  analogWrite(right_pwm,108); //The green state is already the maximum speed and this is safer for any passengers
}
